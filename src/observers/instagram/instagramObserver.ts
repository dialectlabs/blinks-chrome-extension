...
import { getInstagramStylePreset } from './instagramStyles';
...
export function setupInstagramObserver(
  ...
  const initObserver = () => {
    ...
    handleExistingNodes(instagramRoot, config, callbacks, mergedOptions);
    ...
    observer.observe(instagramRoot, { childList: true, subtree: true });
...
function handleExistingNodes(
  ...
  // Handle comments
  const comments = root.querySelectorAll('ul[class*="x78zum5"] li');
  ...
  // Handle DMs
  const dms = root.querySelectorAll(
    'div[class*="x78zum5"] div[class*="x1cy8zhl"]',
  );
  dms.forEach((dm) => {
    const links = dm.querySelectorAll('a[href^="https://"]');
    links.forEach((link) => {
      handleNewNode(link, config, callbacks, options).catch(console.error);
    });
  });
...
function createAction({
  ...
  const isDarkMode = document.body.classList.contains('_aa4d');
  const stylePreset = getInstagramStylePreset(isDarkMode);
