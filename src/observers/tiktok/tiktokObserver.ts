...
import { getTikTokStylePreset } from './tiktokStyles';
...
export function setupTikTokObserver(
  ...
  const initObserver = () => {
    ...
    handleExistingNodes(tiktokRoot, config, callbacks, mergedOptions);
    ...
    observer.observe(tiktokRoot, { childList: true, subtree: true });
...
function handleExistingNodes(
  ...
  const descriptions = root.querySelectorAll('.video-meta-description');
  ...
  // Handle comments
  const comments = root.querySelectorAll('.comment-item');
  ...
  // Handle user bios
  const userBios = root.querySelectorAll('.share-desc');
  userBios.forEach((bio) => {
    const links = bio.querySelectorAll('a[href^="https://"]');
    links.forEach((link) => {
      handleNewNode(link, config, callbacks, options).catch(console.error);
    });
  });
...
function createAction({
  ...
  const stylePreset = getTikTokStylePreset(isDarkMode);
