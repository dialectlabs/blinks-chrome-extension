...
import { getDocsStylePreset } from './docsStyles';
...
export function setupDocsObserver(
  ...
  const initObserver = () => {
    ...
    handleExistingNodes(docsRoot, config, callbacks, mergedOptions);
...
function handleExistingNodes(
  ...
  // Handle main document area
  const documentLinks = root.querySelectorAll('.kix-lineview-content a');
  documentLinks.forEach((link) => {
    handleNewNode(link, config, callbacks, options).catch(console.error);
  });

  // Handle comments
  const commentLinks = root.querySelectorAll('.docos-streampane-content a');
...
function createAction({
  ...
  const isDarkMode = document.body.classList.contains('docs-gm-dark');
  const stylePreset = getDocsStylePreset(isDarkMode);
