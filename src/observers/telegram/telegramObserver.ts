...
import { getTelegramStylePreset } from './telegramStyles';
...
export function setupTelegramObserver(
  ...
  const initObserver = () => {
    window.processedLinks = new Set<string>();
    handleExistingNodes(telegramRoot, config, callbacks, mergedOptions);
  ...
  refreshRegistry().then(() => {
    ...
    // Listen for URL changes (Telegram uses pushState)
...
function createAction({
  ...
  const stylePreset = getTelegramStylePreset(isDarkMode);
