...
import { getPinterestStylePreset } from './pinterestStyles';
...
export function setupPinterestObserver(
  ...
  const initObserver = () => {
    window.processedLinks = new Set<string>();
    handleExistingNodes(pinterestRoot, config, callbacks, mergedOptions);
    ...
    observer.observe(pinterestRoot, { childList: true, subtree: true });
  ...
  refreshRegistry().then(() => {
    ...
    // Listen for URL changes (Pinterest uses pushState)
...
function createAction({
  ...
  const isDarkMode = document.body.classList.contains('themed-dark');
  const stylePreset = getPinterestStylePreset(isDarkMode);
